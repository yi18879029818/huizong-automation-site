...
export default function ForkliftHero({ page }) {
  ...
  return (
    <section className="forklift-hero overflow-hidden bg-primary text-white">
      <div className="mx-auto grid max-w-[1440px] gap-8 px-6 py-10 md:px-12 md:py-14 lg:grid-cols-[minmax(0,0.72fr)_minmax(0,1.28fr)] lg:items-center lg:gap-10 lg:py-12 xl:gap-14 xl:py-14">
        <div className="relative z-10">
          ...
          <p className="mt-4 max-w-lg text-base leading-[1.55] text-white/70 md:text-lg">
            {summary}
          </p>
          ...
          <div className="mt-7 grid max-w-2xl grid-cols-1 gap-px overflow-hidden rounded-2xl border border-white/15 bg-white/15 sm:grid-cols-3">
            {DEFAULT_METRICS.map(([value, label]) => (
              <div className="bg-primary/78 px-3 py-3 backdrop-blur-sm" key={label}>
        ...
        <div className="relative min-h-[380px] overflow-hidden rounded-[28px] border border-white/20 bg-white/[0.08] p-1.5 shadow-[0_26px_80px_rgba(0,0,0,0.24)] backdrop-blur-sm md:min-h-[500px] lg:min-h-[600px]">
          <img
            alt="Forklift Stacker AGV moving a pallet inside a high-rack warehouse"
            className="h-full min-h-[368px] w-full rounded-[22px] object-cover object-[86%_center] md:min-h-[488px] lg:min-h-[588px]"
            fetchPriority="high"
